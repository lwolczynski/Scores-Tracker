    ...
    return {
        ...
        async getScore() {
            await axios({
                ...
                .then((response) => {
                    for (let score of response.data.scores)
        ...
        initiatePlayerRows() {
            if (this.editable) {
                for (let score of this.scores)
                    this.createEditablePlayerRow(score);
            } else {
        ...
        createFixedPlayerRow(score) {
            ...
            for (let i = 0; i < trs.length; i++) {
                ...
                if (score.scoring[keys[i]] !== undefined) {
                    td.innerHTML =
                        score.scoring[keys[i]] === null ?
                        `&nbsp;` :
                        `${score.scoring[keys[i]]}`;
                } else {
        ...
        createScoreInput(score, key) {
            ...
            setInputFilter(input, function(value) {
                ...
                ); // Allow only integer between 0 and 99
            });
            ...
            axios({
                ...
                .then((response) => {
                    if (response.data.message.tag === "success") {
                        ...
                        this.table
                            .querySelectorAll(`[data-score_id='${score.id}']`)
